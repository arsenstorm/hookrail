# Solid Queue/Cache/Cable live in the primary database (single DATABASE_URL everywhere),
# so their tables are created by a regular migration instead of the separate-database
# schema files Rails generates (db:prepare never loads those into a shared, non-empty DB).
# Transcribed from db/{queue,cache,cable}_schema.rb, which this migration replaces.
